...
datasets=("avatar" "evalplus" "codenet")
#datasets=("codenet")
...
sonar_c_cpp() {
  ...
  mkdir "Code"
  if [[ "$dir" == *"fix-1"* ]]; then
    dir_org="${dir//fix-1/fix-0}"
    cp -r "$dir_org"/* "Code/"
  fi
  cp -r "$dir"/* "Code/"
  find Code/ -maxdepth 1 -type f \( -name '*.c' -o -name '*.cpp' -o -name '*.c++' \) -exec bash -c 'ext="${1##*.}"; dir="$(dirname "$1")"; base="$(basename "$1" .$ext)"; mkdir -p "$dir/$base" && mv "$1" "$dir/$base/$base.$ext"' _ {} \;
  cp ../CMakeLists.txt .
  mkdir build
  cd build || exit
...
run_all <data_path> "outputs/lit/fix-1" "lit-after"
run_all <data_path> "outputs/lit/fix-0" "lit-before"
